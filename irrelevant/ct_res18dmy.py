...
class Model(ct_res18.Model):
    def loss(self, x, y, *, adv=True): # NOTE, true by default
        ...
        if not adv:
            ...
            xrg, rg = projectedGradientDescent(self, images, labels,
                    eps=0.03, alpha=2./255., maxiter=7, verbose=False,
                    device=device, targeted=False, unbound=False, rinit=False, B_UAP=True)
            # forward the PGD adversary
            self.train()
            output, loss = self.loss(xr, labels, adv=False)
            outputg, lossg = self.loss(xrg, labels, adv=False)
            loss = alpha * loss + (1.0-alpha) * lossg
