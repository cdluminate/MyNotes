...
\begin{table}[h!]
\caption{Cheatsheet 索引表}
    \centering
    \resizebox{\textwidth}{!}{% 自动缩放以适应页面宽度
    \begin{tabular}{cl@{\hspace{1.5cm}}cl} % 中间增加 1.5cm 间距
        \toprule
        链接 & 表格名称 & 链接 & 表格名称 \\
        \midrule
        \ref{tab:mbti-jung} & MBTI - 荣格八维映射表 (Function Stacks) & \ref{tab:cog-function-critical} & 认知功能的6*批判家长(Critical Parents)表现 \\
        \ref{tab:cog-function-roles} & 认知功能的角色表（含标准定义符合度） & \ref{tab:cog-function-trickster} & 认知功能的7*盲点(Trickster)表现 \\
        \ref{tab:cog-function-normal} & 认知功能的正常表现 & \ref{tab:cog-function-demon} & 认知功能的8*恶魔(Demon)表现 \\
        \ref{tab:cog-function-child} & 认知功能的3*永恒之子(Child)表现 & \ref{tab:cog-function-demon-redemption} & 认知功能的8*恶魔(Demon)整合 \\
        \ref{tab:cog-function-inferior} & 认知功能的4*阿尼玛(Inferior)表现 & \ref{tab:cog-function-loop} & 认知功能的防御机制/长期内耗循环(Loop) \\
        \ref{tab:cog-function-inferior-remedy} & 极端压力/抓取状态(Grip)的解药 & \ref{tab:cog-function-loop-remedy} & 长期内耗循环(Loop)的解药 \\
        \ref{tab:cog-function-opposing} & 认知功能的5*反向作用(Opposing/Nemesis)表现 & & \\
        \bottomrule
    \end{tabular}}
\end{table}
...
\begin{table}[h!]
\caption{认知功能的5*反向作用(Opposing/Nemesis)表现：防御与质疑。
...
\midrule
Ne & INTJ, INFJ & “别扯那些没用的” & \textbf{厌恶变数}：固执地坚持原计划，拒绝听取其他可能性，认为那是干扰 & \textbf{怀疑意图}：总觉得别人背信弃义，或者怀疑别人的忠诚度\tabularnewline
...
\begin{table}[h!]
\caption{状态(State) vs. 特质(Trait)：预期管理表。
...
\begin{table}[h!]
\caption{全谱系战略应对表：针对 8 种内耗循环(Loop)的社交博弈策略。
...
\begin{table}[h!]
\caption{全谱系社会控制矩阵：资本主义如何无死角收割 8 种认知循环(Loop)。
...
\begin{table}[h!]
\caption{INTJ 的四重人格状态切换表：你体内的隐藏角色。
...
\begin{table}[h!]
\caption{社交狩猎矩阵：PUA与精神控制是如何利用认知功能漏洞的。
...
\begin{table}[h!]
\caption{反PUA防御矩阵：如何利用认知功能进行框架重置。
