...
new_weight_var = lambda shape: tf.Variable(tf.truncated_normal(shape, stddev=0.1))
new_bias_var   = lambda shape: tf.Variable(tf.constant(0.1, shape=shape))
new_conv2d     = lambda x, W : tf.nn.conv2d(
  x, W, strides=[1,1,1,1], padding='SAME')
new_maxpool_2x2= lambda x    : tf.nn.max_pool(x, ksize=[1,2,2,1],
  strides=[1,2,2,1], padding='SAME')

def mnist_deepconvnet(x):
  ''' build a deep convolutional netowrk
      x: tf.placeholder, size 784
  '''
  with tf.name_scope('reshape'):
    x_image = tf.reshape(x, [-1, 28, 28, 1])
  with tf.name_scope('conv1'):
    W_conv1 = new_weight_var([5,5,1,32])
    b_conv1 = new_bias_var([32])
    h_conv1 = tf.nn.relu(new_conv2d(x_image, W_conv1) + b_conv1)
  with tf.name_scope('pool1'):
    h_pool1 = new_maxpool_2x2(h_conv1)
  with tf.name_scope('conv2'):
    W_conv2 = new_weight_var([5,5,32,64])
    b_conv2 = new_bias_var([64])
    h_conv2 = tf.nn.relu(new_conv2d(h_pool1, W_conv2) + b_conv2)
  with tf.name_scope('pool2'):
    h_pool2 = new_maxpool_2x2(h_conv2)
  with tf.name_scope('fc1'):
    h_pool2_flat = tf.reshape(h_pool2, [-1, 7*7*64])
    W_fc1 = new_weight_var([7*7*64, 1024])
    b_fc1 = new_bias_var([1024])
    h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)
  with tf.name_scope('dropout'):
    keep_prob = tf.placeholder(tf.float32)
    h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)
  with tf.name_scope('fc2'):
    W_fc2 = new_weight_var([1024, 10])
    b_fc2 = new_bias_var([10])
    y_ = tf.nn.softmax(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)
  return y_, keep_prob

y_, keep_prob = mnist_deepconvnet(x)
