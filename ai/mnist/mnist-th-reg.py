...
class Net(th.nn.Module):
    def __init__(self):
        ...
        self.fc1 = th.nn.Linear(784, 384) # affine operation
        self.fc2 = th.nn.Linear(384, 1)
    def forward(self, x):
        x = self.fc1(x)
        x = F.relu(x)
        x = self.fc2(x)
...
net = Net()
print(net)
crit = th.nn.MSELoss() #th.nn.CrossEntropyLoss()
...
def transform(images, labels):
    ...
    labels = Variable(th.from_numpy(labels.reshape(-1).astype(np.double)), requires_grad=False)
...
for i in range(1000+1):
    ...
    pred = out.data.view(1,64) - labels.data.view(1,64)
    print(pred)
    correct = 0 
